...
- `icons` フォルダ: 各種サイズのアイコン画像を格納。
- 権限 (`permissions`): `activeTab`, `clipboardWrite`, `scripting` (ページ内のメタデータ抽出処理のため)
- Icons: `icons/` フォルダ内の 16x16, 32x32, 48x48, 128x128 の画像を使用。
- Action: アイコンクリック時に `popup.html` を表示し、ツールバーアイコンも表示されるよう設定。
...
- **Elsevier (ScienceDirect) 等の動的レンダリングサイト**: Elsevier社のScienceDirectなどの一部の学術サイトでは、標準的な `<meta>` タグ（`citation_author` 等）や構造化データが初期HTML内に存在せず、JavaScriptによって難読化された状態で遅延読み込みされる場合があります。このようなサイトでは本拡張機能から論文データを抽出できず、「メタデータがない場合のフォールバック（ページタイトルのみ）」として処理されます。将来的に専用のスクレイパーAPIを利用するか、ScienceDirect特有のAPI連携による解決を検討する必要があります。
